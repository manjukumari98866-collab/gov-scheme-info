...
### Local Offline/Online Mode

The frontend ships with bundled service data, so it renders without the backend. When the backend is reachable, it syncs up to 1000 schemes and keeps the latest successful response in browser storage for offline use. Run the frontend through HTTP (not `file://`) so the PWA service worker can cache the app:
...
For a deployed backend, set `window.BHARAT_API_BASE_URL` before `api-client.js` in `index.html`, or set `localStorage.setItem('gov-api-base-url', 'https://your-domain.example/api')` in the browser console. Do not put MongoDB passwords or JWT secrets in frontend files.

Firebase web configuration values may be visible in frontend code by design, but Firebase Console must restrict authorised domains and enable only the sign-in providers you use. Keep all server credentials in `backend/.env`.
